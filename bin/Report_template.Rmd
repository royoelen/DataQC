...
`DataQC` pipeline has prepared `r ncol(cov) - 1` covariates: 10 first genetic PCs, 100 first expression PCs`r if (ncol(cov) - 1 > 110){paste0(" and ", (ncol(cov) - 1) - 110, " additional covariates which were specified by you."} else {"."}`

`ŗ if (ncol(cov) - 1 > 110){paste0("The names of additional covariates are: ", colnames(cov)[-1:110], ".")}`

```{r, message = FALSE, warning = FALSE, echo = FALSE}
if (ncol(cov) > 111){
  
  cov_specific <- cov[, -c(2:111), with = FALSE]
  
  cov_specific <- melt(cov_specific, id.vars = "SampleID")

  colnames(cov_specific) <- c("SampleID", "covariate", "value")

  ggplot(cov_specific, aes(x = value)) + geom_histogram(colour = "white") + facet_wrap(~ covariate) + theme_bw()
  
  # Check how many are binary:
  is_binary <- function(x){
    if (all(x %in% c(0, 1))){return(TRUE)} else {
      return(FALSE)
      }
    }

  levels_cov <- apply(cov[, -c(2:111), with = FALSE], 2, is_binary)

  if (any(levels_cov == TRUE)){knitr::kable(apply(cov_specific[, levels_cov], 2, table))}

  }
```

`r if (ncol(cov) > 111 & any(levels_cov == TRUE)){"Overview of binary covariates."}`
...
If you still observe clusters in the data, that could indicate that you still have different genotype batches in the data. If you know genotype batches splitting data is unoptimal, you should specify the batch as additional covariate in `--AdditionalCovariates` argument.
