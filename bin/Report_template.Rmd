...
- If you see the majority of the dots on 1-0 diagonal, this suggest that your input genotypes have incorrect encoding of the alleles for all SNPs.
- If you see inverted "V" shape, this suggests that there are some subset of SNPs which have alleles mixed.

In both of the cases you should check if the input `.bed/.bim/.fam` files are correct and there are no errors made in generating and/or processing those.
...
![**Fig 12. Expression of X/Y chromosome genes.** On the x-axis is the mean normalised expression level of X chromosome genes, on the y-axis is the mean of the normalised expression levels of Y chromosome genes. For the better separation of groups, minimal X and Y value was extracted from each value on X and Y axes. Point type indicates the genetic sex and there should be no samples on the X-Y diagonal. Samples which are in the wrong sample cloud, are likely sample mixups and were automatically removed from the data. Samples which are in the shaded area, are likely contaminated and automatically removed. Start of the separating line is set at median Y value of female samples and median X value of male samples.](outputfolder_exp/exp_plots/SexSpecificGenes.png) 
...
**Fig 12. Expression of X/Y chromosome genes.** Not available!
