...
`r if(file.exists("outputfolder_gen/gen_plots/SexCheck.png")) {"\\begin{comment}"}`
Step skipped since no X chromosome was found in the genotype data
`r if(!file.exists("outputfolder_gen/gen_plots/SexCheck.png")) {"\\end{comment}"}`
...
Samples which expression does not align with genetic sex are potential sample mixups. These are outlined with red and automatically removed from further analyses. If the X chromosome was not present, expression is compared to reported
sex instead.
