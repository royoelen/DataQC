...
overview_table <- fread("outputfolder_gen/data/summary_table.txt")
...
![Statistic of outlierness with used threshold.](outputfolder_gen/plots/PC_dist_outliers_S.png)  

![PCA with all samples, outliers outlined.](outputfolder_gen/plots/PCA_outliers.png)  

![PCA with all samples, outliers removed.](outputfolder_gen/plots/Target_PCs_postQC.png)  

#### Loadings of PCs

<!-- ![Loadings of PCs.](outputfolder_gen/plots/Target_PCs_postQC_Loadings.png)  -->

#### Genotype data projected to 1000G superpopulations

1. If you observe a few samples which are different anchestry compared to the majority of samples (e.g. majority of the samples are of EUR ancestry and there are few samples from AFR or SAS), then remove ethnic outliers and re-run the GenotypeQc pipeline without those samples.
2. If you observe many (>100) samples which are different anchestry compared to the majority of samples (your dataset consists of individuals from several ancestries) then please split your genotype data based on ancestry and rerun this and following pipelines on those datasets separately.

The file `1000G_PC_projections.txt` has been written to the your output folder under `data`. This can be used to manually extract the outlier samples based on their PC projections into 1000G superpopulations. You can use plink/plink 1.9/plink 2.0 to filter samples from initial .bed/.bim/.fam files.

![Genotype samples projected to 1000G superpopulations](outputfolder_gen/plots/SamplesPCsProjectedTo1000G.png)  
...
library(dplyr)
pops <- fread("outputfolder_gen/data/PopAssignResults.txt")
...
mutate(Confidence = case_when(Difference > 2 ~ "confident", Difference <= 2 ~ "not_confient"))
...
![PCA with all samples, outliers outlined.](outputfolder_exp/exp_plots/PCA_before_and_after.png)  

Explained variance of final PCs (normalised, log-transformed, Z-transformed and inverse normal transformed expression matrix).
