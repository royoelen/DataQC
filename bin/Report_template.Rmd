...
1. If you observe clusters with limited numbers of samples (e.g. N<500) and a clear separation based on PC1 or PC2, you can consider using the data as it is: first genetic PCs will be used as covariates in the meta-analysis and such batch effects will be mostly accounted for. In that case, you should re-run the `DataQC` pipeline with large SD value, e.g. `--SDthresh 10`, to keep all the data in.
...
Samples in the shaded area are automatically removed. The area is defined by an angle of 30 degrees centered over the x-y diagonal.

Samples for which the sex-specific expression does not align with genetic sex are potential sample mix-ups.
These are outlined with red and automatically removed from further analyses. If the X chromosome was not present,
expression is compared to reported sex instead.
