...
```
### Allele frequency concordance with reference

In the following plot you see the comparison of allele frequencies between input genotypes and 1000G phase 3 reference panel. This lookup is done on SNP IDs, therefore many SNPs will be excluded. However, even with the subset of SNPs we can check if the allele frequencies correlate with each other. We should see that large majority of the variants are on the 0-1 diagonal and strong correlation between the allele frequencies of two datasets.
...
In both of the cases you should check if the input `.bed/.bim/.fam` files are correct and there are no errors made in generating and/or processing those. If you do not observe any issues with input, please contact with the lead analyst for advice.
...
**Fig 1. Comparison of allele frequencies between input genotypes and 1000G phase 3 reference. Comparison is restricted to up to ~1.7M HapMap3 or UK Biobank SNPs as provided by `bigsnpr` R package.** Ambiguous variants (A/T, C/G SNPs) and variants whose alleles seem to be from opposite strands in two datasets were removed from this analysis. Out of `r nrow(target)` variants in the input genotype data, `r nrow(comb)` overlap with `r nrow(ref)` variants in the reference data. Out of those, `r nrow(comb[(comb$ref_REF != comb$target_REF & comb$ref_REF != comb$target_ALT)])` are removed from this lookup due to difference of strand between reference and input and `r nrow(comb2[((comb2$ref_REF == "A" & comb2$ref_ALT == "T") | (comb2$ref_REF == "T" & comb2$ref_ALT == "A") | (comb2$ref_REF == "C" & comb2$ref_ALT == "G") | (comb2$ref_REF == "G" & comb2$ref_ALT == "C")), ])` are removed because these are ambiguous variants. The Pearson R of allele frequencies is `r res_cor`, which suggests `r if(res_cor > 0 & res_cor < 0.5){quality_res <- "that there is something clearly wrong with your input genotypes. You should double-check your input data."} else if (res_cor > 0.5 & res_cor < 0.8){quality_res <- "that you might have something wrong with your input genotypes. Allele frequencies between two datasets correlate, however we would expect higher correlation. You should double-check your input data and contact with lead analysts for advice."} else if (res_cor > 0.8){quality_res <- "that your input data does not have major problems with alleles."} else if (res_cor < 0){quality_res <- "you might have systematic issues with allele encoding in your input genotypes. You should double-check your input data."}; quality_res`
...
![**Fig 2. Distribution of F-values representing X chromosome heterozygosity.** Genetically determined sex thresholds are shown as vertical lines. Samples with the F-value between those two lines are potentially contaminated samples and are automatically excluded.](outputfolder_gen/gen_plots/SexCheck.png)
...
**Fig 2. Distribution of F-values representing X chromosome heterozygosity.** Not available!
...
![**Fig 3. Distribution of heterozygosity rate.** Thresholds for declaring excess/depleted heterozygosity for sample (mean+/-3SD) are shown as vertical lines. ](outputfolder_gen/gen_plots/HetCheck.png)  
...
![**Fig 4. Genotype samples projected to 1000G superpopulations.** First two plots show only target dataset in the 1000G PC space. Rest of the plots visualise target dataset, as well as samples from 1000G superpopulations by plotting first 10 genotype PCs.](outputfolder_gen/gen_plots/SamplesPCsProjectedTo1000G.png)  
...
**Fig 5. Euclidean distance between target samples and samples from each 1000G superpopulation.** 
...
![**Fig 6. Statistic of outlierness with threshold used to remove ethnic outliers.**](outputfolder_gen/gen_plots/PC_dist_outliers_S.png)  
...
![**Fig 7. PCA with all samples, outliers outlined with red.** Different shades of red indicate different classes of outliers: difference in SD from mean of PC1 and PC2 (pink), statistic of outlierness S (red), or both (dark red). Dashed lines indicate the SD thresholds to consider samples to outliers.](outputfolder_gen/gen_plots/PCA_outliers.png)  

![**Fig 8. PCA with all samples, outliers removed.** You should not see any dramatic outliers any more. These are the PCs which will be used as covariates in the eQTL meta-analysis.](outputfolder_gen/gen_plots/Target_PCs_postQC.png)  
...
![**Fig 9. Loadings of 10 first PCs.**](outputfolder_gen/gen_plots/Target_PCs_postQC_Loadings.png)
...
![**Fig 10. PCA on all samples** Genetic sex of the individual is shown as shape of the dot, outliers from all iterations are outlined as red.](outputfolder_exp/exp_plots/PCA_before.png)
...
![**Fig 11. MDS on all samples** Genetic sex of the individual is shown as shape of the dot, outliers are outlined as red.](outputfolder_exp/exp_plots/MDS_before.png)
...
![**Fig 12. PCA on all samples, outliers removed.** Genetic sex of the individual is shown as shape of the dot.](outputfolder_exp/exp_plots/PCA_after.png) 

#### Explained variance of expression PCs

![**Fig 13. PCA scree plot for normalised and inverse normal transformed expression matrix.** Outlier samples were removed before re-calculating PCA.](outputfolder_exp/exp_plots/PCA_final_scree_plot.png)
...
**Fig 14. Cumulative explained variance for first 100 PCs.**
...
![**Fig 16. Expression of X/Y chromosome genes.** On the x-axis is the mean normalised expression level of X chromosome genes, on the y-axis is the mean of the normalised expression levels of Y chromosome genes. For the better separation of groups, minimal X and Y value was extracted from each value on X and Y axes. Point type indicates the genetic sex and there should be no samples on the X-Y diagonal. Samples which are in the wrong sample cloud, are likely sample mixups and were automatically removed from the data. Samples which are in the shaded area, are likely contaminated and automatically removed. Start of the separating line is set at median Y value of female samples and median X value of male samples.](outputfolder_exp/exp_plots/SexSpecificGenes.png) 
...
**Fig 17. Expression of X/Y chromosome genes.** Not available!
...
The eQTLGen DataQC pipeline makes extensive use of the following bioinformatics packages:
