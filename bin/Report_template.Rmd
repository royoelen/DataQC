...
On the following plots, 8 first PCs are calculated on normalised and log~2~-transformed expression matrix. Samples are declared as outliers based on first two PCs, when they deviate from the mean value by 3 standard deviations (SDs). Analysis was run iteratively, data was pre-processed and PCA calculated until there were no further expression otliers to remove.

![PCA on all samples, outliers from first iteration outlined.](outputfolder_exp/exp_plots/PCA_before.png)

These are 10 first expression PCs, calculated on fully processed expression data (normalised, log~2~-transformed, Z-transformed, INT-transformed) after removal of outlier samples. You should not see any clear outliers any more.

![PCA on all samples, outliers removed.](outputfolder_exp/exp_plots/PCA_after.png) 
...
![PCA scree plot for normalised, inverse normal transformed and centered expression matrix. Outlier samples were removed before re-calculating PCA.](outputfolder_exp/exp_plots/PCA_final_scree_plot.png)
...
p
```
First 5 PCs explain cumulatively `r overview_pcs_cumulative$cumulative[5]`% of variance.

First 10 PCs explain cumulatively `r overview_pcs_cumulative$cumulative[10]`% of variance.

First 20 PCs explain cumulatively `r overview_pcs_cumulative$cumulative[20]`% of variance.

First 50 PCs explain cumulatively `r overview_pcs_cumulative$cumulative[50]`% of variance.

First 100 PCs explain cumulatively `r overview_pcs_cumulative$cumulative[100]`% of variance.
